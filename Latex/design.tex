...
\subsubsection*{Carbon Fiber Poles \& Supports}
\red{Screw hole orientation}
		
\subsubsection*{Servos}
		
\subsubsection*{Carbon Fiber Poles and Supports}
\red{Repeated name?}
The hybrid frame was created using a combination of two 12mm carbon fibre poles (front and back) and 3D printed mounts and supports. As gear systems, mounts and legs needed to be attactched, all holes in the rod due to fastening were drilled parallel to the base of the drone in order to minimise any reductions in strength (as the top and bottom of the poles have tesnsile and compressive stresses when hovering).
...
Validation, small tests...etc

\subsubsection*{Testing Mechanisms}
Table, rope, small courtyard tests, motor tests, wire and solder test...etc

\subsubsection*{Iterative Design}
\todomessage{This list would look better as its own subsection, maybe as 4.6?}
Through testing, a number of problems with the aircraft have been tweaked and resolved

\begin{table}[!hl]
	\begin{tabular}{l l}
		Thrust: & Thrust tests were performed on each selected propellor in order to get a quantitive ranking on their effectiveness.\\
		Vibrations: & Numerous vibration related problems and set-backs were encountered during testing and evaluation. An assortment of nyloc-nuts and spring-washers were utilized in order to fasten components together in a vibration resistant manner.\\
		Motor Mounts: & \\
		Gears: & \\
		Wires and Soldering: & (this one is very recent, have good plans for resolution)\\
		Radio Failure & \\
		Motor Failure: & \\
		Power Module Failure: & \\
		GPS and Compass: & (basically how we moved the GPS away from the ESC in order to avoid the magnetic field of the alternating current)
	\end{tabular}
\end{table}

\subsubsection*{Most Recent Results}
(Hopefully Transition)
